...
const Steper: React.FC<SteperInterface> = ({ Stepers }) => {
  return (
    <div>
      {" "}
      <ol className='relative w-full text-gray-500 border-s border-gray-200 dark:border-gray-700 dark:text-gray-400 px-2 font-heading '>
        {Stepers?.map((item: any, index: number) => {
          return (
            <li className='mb-10 ms-6'>
              ...
              <h3 className=' leading-tight font-semibold text-xl text-neutral-700'>
                {item?.name}
              </h3>
              {item?.discriptions}
            </li>
