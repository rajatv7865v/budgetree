...
import Logo from "../../../public/assets/budgetree_logo.png";
...
const Navbar = () => {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 inset-x-0 h-14 w-full border-b border-transparent z-[99999] select-none bg-white text-black",
        scroll &&
          "border-background/80 bg-background/40 backdrop-blur-md bg-white"
      )}
    >
              ...
              <NavigationMenuList>
                {NAV_LINKS.map((link) => (
                  <NavigationMenuItem key={link.title}>
                    {link.menu ? (
                      <>
                        <NavigationMenuTrigger className='text-[#1e4aa0]'>
                          {link.title}
                        </NavigationMenuTrigger>
                        <NavigationMenuContent>
                          <ul
                            className={cn(
                              "grid gap-1 p-4 md:w-[400px] lg:w-[500px] rounded-xl text-[#1e4aa0]",
                              link.title === "Features"
    ...
    <li>
      <NavigationMenuLink asChild>
        <Link
          ...
          ref={ref}
          className={cn(
            "block select-none space-y-1 rounded-lg p-3 leading-none no-underline outline-none transition-all duration-100 ease-out hover:bg-accent hover:text-[#1e4aa0] focus:bg-accent focus:text-[#1e4aa0]",
            className
          )}
          {...props}
        >
          <div className='flex items-center space-x-2 text-neutral-300'>
            ...
            <h6 className='text-sm font-medium !leading-none hover:text-[#1e4aa0]'>
              {title}
            </h6>
          </div>
          <p
